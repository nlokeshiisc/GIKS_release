...
def generate_patient(x, v, num_treatments, treatment_type, treatment_selection_bias, dosage_selection_bias,
                     scaling_parameter, noise_std):
    outcomes = []
    dosages = []

    assert num_treatments == 1, "We support three treatment types. But the networks handle only univariate treatments"

    for _ in range(num_treatments):
        if (treatment_type == 0):
            b = 0.75 * np.dot(x, v[treatment_type][1]) / (np.dot(x, v[treatment_type][2]))
            ...
            y = get_patient_outcome(x, v, treatment_type, dosage, scaling_parameter)

        elif (treatment_type == 1):
            optimal_dosage = np.dot(x, v[treatment_type][2]) / (2.0 * np.dot(x, v[treatment_type][1]))
            alpha = dosage_selection_bias
            dosage = np.random.beta(alpha, compute_beta(alpha, optimal_dosage))
            ...
            optimal_dosage = np.dot(x, v[treatment_type][1]) / (2.0 * np.dot(x, v[treatment_type][2]))
            alpha = dosage_selection_bias
            dosage = np.random.beta(alpha, compute_beta(alpha, optimal_dosage))
            if (optimal_dosage <= 0.001):
                dosage = 1 - dosage
    ...
    treatment_type = np.random.choice(num_treatments, p=softmax(treatment_coeff))

    return treatment_type, dosages[treatment_type], outcomes[treatment_type] + np.random.normal(0, noise_std)
...
def get_split_indices(num_patients, patients, treatments, test_fraction, random_state):
    ...
    test_sss = StratifiedShuffleSplit(n_splits=1, test_size=num_test_patients, random_state=random_state)
    train_indices, test_indices = next(test_sss.split(patients, treatments))
...
class TCGA_Data():
    def __init__(self, args, dataset_num, treatment_type):
        #np.random.seed(3)
        ...
        self.tcga_data = pickle.load(open(fcf_dir / 'dataset/tcga/tcga.p', 'rb'))
        ...
        # self.v is required for handling many treatments
        ...
        self.dataset = self.generate_dataset(self.patients, self.num_treatments, self.treatment_type)
    
    def __str__(self) -> str:
        return f"TCGAWithTreatment={self.treatment_type}"
    ...
    def generate_dataset(self, patient_features, num_treatments, treatment_type):
        tcga_dataset = dict()
        tcga_dataset['x'] = []
        tcga_dataset['y'] = []
        tcga_dataset['t'] = []
        ...
        for patient in patient_features:
            t, dosage, y = generate_patient(x=patient, v=self.v, num_treatments=1,
        ...
        # We split our own validation dataaset inside early stopping
        train_indices, test_indices = get_split_indices(num_patients=tcga_dataset['x'].shape[0],
                                                        patients=tcga_dataset['x'],
                                                        treatments=tcga_dataset['t'],
                                                        # validation_fraction=self.validation_fraction,
                                                        test_fraction=self.test_fraction,
                                                        random_state=int(self.dataset_num * 100)) # random_state is reqd to force different train test idxs in each seed
