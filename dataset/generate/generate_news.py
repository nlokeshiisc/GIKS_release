...
path = 'dataset/generate/news_pp.npy'
save_path = 'dataset/news_4/tr_h_{:02}_te_h_{:02}'.format(train_h, test_h)
news = np.load(path)
...
def news_matrix():
    ...
    # get data matrix
    for zid in range(num_data // B_per_i):
        x = news[zid, :]
        for bb in range(B_per_i):
            ...
            train_matrix[zid, num_feature+1] = y
            train_matrix[zid, 1: num_feature+1] = x_tensor
    ...
    for zid in range(num_data // B_per_i):
        ...
        for bb in range(B_per_i):
            ...
            data_matrix[zid, 0] = t
            data_matrix[zid, num_feature+1] = y
            data_matrix[zid, 1: num_feature+1] = x_tensor
...
for _ in range(20):
    ...
    idx_list = torch.arange(num_data).view(-1, B_per_i)
    idx_list = idx_list[torch.randperm(num_data // B_per_i)]
    idx_train = idx_list[0:2000].flatten()
    idx_test = idx_list[2000:].flatten()
